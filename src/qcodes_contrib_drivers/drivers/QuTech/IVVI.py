...
class IVVI(VisaInstrument):
    ...
    def __init__(self, name, address, reset=False, numdacs=16, dac_step=10,
        ...
        self.resolution = 16
        self.dacsteps = (2**self.resolution)-1 # 0-65535 (655536=2**16 discrete steps)
        ...
        self.add_parameter('dac_steps',
                           label='DAC steps',
                           get_cmd=self._get_dacsteps)
        ...
        for i in range(1, numdacs + 1):
            self.add_parameter(
                'dac_step{}'.format(i),
                label='DAC {}'.format(i),
                get_cmd=self._gen_ch_get_func(self._get_dacstep, i),
                set_cmd=self._gen_ch_set_func(self._set_dacstep, i),
                vals=vals.Numbers(0,
                                  self.dacsteps),
                step=dac_step/self.dac_quanta,
                inter_delay=dac_delay)
    ...
    def get_idn(self):
        ...
        """
        idparts = ['QuTech', 'IVVI', 'N.A.', self.version()]
    ...
    def linspace(self, start: float, end: float, samples: int, polarity: str):
        ...
        the requested range. Number of samples is changed to ensure
        even dac step spacing and closest fit to the input linspace.
        Args:
            ...
            polarity: 'BIP', 'NEG', or 'POS' 
        Returns:
            list of voltages in millivolts suitable for the ivvi DAC.
            Voltages are the closest match to [start: end] but can be outside.
            Number of samples is not conserved, but matched as closely
            as possible.
            Voltages are evenly spaced.
            Voltages align with the DAC quantisation.
            Voltages will never exceed the DAC range.
        """
        ...
        if samples < 2:
            raise ValueError('samples: needs to be 2 or more')
        polstring = ['BIP', 'POS', 'NEG']
        if polarity not in polstring:
            raise ValueError('polarity: must be \'BIP\', \'POS\' or \'NEG\'')

        # Define dac mapping on voltage, depending on polarity
        if polarity == 'BIP':
            dacoffset = self.dacsteps/2
        elif polarity == 'NEG':
            dacoffset = -self.dacsteps
        elif polarity == 'POS':
            dacoffset = 0

        # Define start and end dac value
        startdacsync = round((start / self.dac_quanta) + dacoffset)
        enddacsync = round((end / self.dac_quanta) + dacoffset)

        # Determine total dac steps and spacing
        totaldacsteps = enddacsync-startdacsync
        spacing = (totaldacsteps)/(samples-1)

        # Throws error if spacing is smaller than one dacstep.
        if abs(spacing) < 1:
            msg = ('Number of steps too high. Maximum is ' + str(abs(int(enddacsync-startdacsync))) + '.')
            raise ValueError(msg)

        spacinground = round(spacing)
        #print(startdacsync,enddacsync,int(totaldacsteps/spacing))

        # Determines numer of steps most closely resembling the input linspace
        beststeps = int(round(totaldacsteps/spacinground))
        #print(beststeps)

        # Centers the new DAC linspace by shifting the start and ending dac value accordingly
        startdacsynccentered = startdacsync + int(round((enddacsync-(startdacsync+beststeps*spacinground))/2))
        enddacsynccentered = startdacsynccentered + beststeps*spacinground
        #print(startdacsynccentered)

        # Defines new linspace in DAC values
        result = np.linspace(startdacsynccentered,enddacsynccentered,beststeps+1)

        # Deletes entries falling outside the DAC range
        resultlims = result[ (result >= 0) & (result <= self.dacsteps) ]
    ...
    def _dacstep_to_bytes(self, dacstep):
        '''
        Converts dacsteps to a list of two bytes

        Input:
            dacstep (float) : integer in 0-(2**16) range
        Output:
            (dataH, dataL) (int, int) : The high and low value byte equivalent
        '''
        bytevalue = int(dacstep)
    ...
    def _bytes_to_dacstep(self, byte_mess):
        '''
        Converts a list of bytes to a list containing
        the corresponding mvoltages
        '''
        ...
        for i in range(self._numdacs):
            # takes two bytes, converts it to a 16 bit int and then divides by
            # the range and adds the offset due to the polarity
            values[i] = int((byte_mess[2 + 2 * i] * 256 + byte_mess[3 + 2 * i]))
        return values

    # Communication with device
    def _get_dacstep(self, channel):
        """
        Returns dac channel dacstep
        channels range from 1-numdacs

        this version is a wrapper around the IVVI get function.
        it only updates
        """
        return self._get_dacsteps()[channel - 1]

    def _set_dacstep(self, channel, dacstep):
        """
        ...
        if self.check_setpoints():
            cur_val = self.get('dacstep{}'.format(channel))
            # dac range in steps / 16 bits FIXME make range depend on polarity
        ...
        if proceed:
            byte_val = self._dacstep_to_bytes(dacstep)
    ...
    def _get_dacsteps(self):
        '''
        Reads from device and returns all dacsteps in a list

        Input:
            None

        Output:
            voltages (float[]) : list containing all dacsteps

        get dacs command takes ~450ms according to ipython timeit
        '''
        self._dacsteps = np.array(range(0,self._numdacs))
        message = bytes([self._numdacs * 2 + 2, 2])
        reply = self.ask(message)
        self._dacsteps = self._bytes_to_dacstep(reply)
        return self._dacsteps
    ...
    def read(self, message_len=None):
        ...
        while bytes_in_buffer < message_len:
            sleepytime = self.dac_read_buffer_sleep() - (time.time() - self.readtimer)
            #print(sleepytime)
            if sleepytime > 0:
