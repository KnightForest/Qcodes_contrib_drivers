...
class Cryomagnetics_4G(VisaInstrument): 
    ...
    def __init__(self, name: str, address: str, axes=('X','Y','Z'), baudrate=9600, reset=False, serial=False, **kwargs,):
...
#        return ans
    ...
    def _get_field(self, channel):
        ...
        try:
            ans = ans.replace('kG','')
            field = round(float(ans) * 0.1,9)
            return field
        except:
            print("Error: 'IMAG?' returned something non-expected:{}".format(self.ans) )
            time.sleep(0.1)
            pass 
